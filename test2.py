...
def recommend_green_bonds_ml(esg_data, sentiment_score, bond_data, output_file,
    ...
    if not os.path.exists(output_file):
        wb = Workbook()
        wb.save(output_file)
    # Handle ESG data and recommendations separately to ensure both are appended
    try:
        # First, try to read existing data
        existing_esg_df = pd.DataFrame()
        existing_recommendations_df = pd.DataFrame()
        
        try:
            existing_esg_df = pd.read_excel(output_file, sheet_name='Esgdata')
        except:
            pass
            
        try:
            existing_recommendations_df = pd.read_excel(output_file, sheet_name='Data')
        except:
            pass
            
        # Append new data to existing data
        if not existing_esg_df.empty:
            combined_esg_df = pd.concat([existing_esg_df, esg_df], ignore_index=True)
        else:
            combined_esg_df = esg_df
            
        if not existing_recommendations_df.empty:
            combined_recommendations_df = pd.concat([existing_recommendations_df, recommendations_df], ignore_index=True)
        else:
            combined_recommendations_df = recommendations_df
            
        # Write back the combined data
        with pd.ExcelWriter(output_file, engine='openpyxl', mode='w') as writer:
            combined_esg_df.to_excel(writer, sheet_name='Esgdata', index=False)
            combined_recommendations_df.to_excel(writer, sheet_name='Data', index=False)
            
    except Exception as e:
        print(f"Error appending data: {e}")
        # Fallback - just write the new data if there's an error
        with pd.ExcelWriter(output_file, engine='openpyxl', mode='w') as writer:
...
# Define the output path on K drive in Abhinav folder
output_directory = r"K:\Abhinav"
output_filename = "ml_based_recommendations13.xlsx"
...
if not os.path.exists(output_directory):
    try:
        ...
        # Fallback to current directory if can't create the folder
        output_file_path = output_filename
        print(f"Using current directory instead: {output_file_path}")
company_name = 'UBS'
...
# Use the new output file path
recommend_green_bonds_ml(esg_data, sentiment_score, bond_data, output_file_path)
